...
这就是任何版式都需要的面包和黄油。下面您会看到承担起颜值和构建内容的各种 Minimal Mistakes 版式。

## 默认版式

所有其它版式都继承自基础版式。基础版式很简单，只是拉取了几个来自 `_includes` 的组件：
...
**备注：** 您不要将 Post 或者 Page 设定为该版式。但是其它版式的构建都设置了 `layout: default` YAML Front Matter 参数来继承它。
{: .notice--warning}

### 版式基本类和用户定义类

每个版式相关的类名添加到 `<body>` 元素后，例如 `<body class="layout--single">`。
...
使用 YAML Front Matter 可以设定定制类使用特定的 CSS 和 JavaScript。完美的“艺术类目录” Post 可以为特定 Page 添加定制的样式。
...
您可以通过在 Page 的 YAML Front Matter 参数中指定 `canonical_url` 选项设定定制的 Canonical URL。例如，如果您使用下列代码：
...
Jekyll 版式可以在纯 Liquid 中对 HTML 进行压缩。要启用在 `_layouts/default.html` 添加 `layout: compress`。

**备注：** 众所周知这会破坏标记和打乱 JavaScript……尤其是如果有行内的 `// comments` 时。基于这个原因，默认是关闭的。
{: .notice--danger}

* [文档](http://jch.penibelst.de/)

## Single 版式

这可能是您最常用到的版式——边栏和主内容区的组合。

**包含：**

* 可选的带标题的 header 图像
* 可选的 header overlay (solid color/image) + 文字和可选的 "call to action" 按钮
* 可选的社会化分享链接模块
* 可选的评论模块
* 可选的相关 Post 模块
* 宽 Page 变化

{% include gallery id="single_layout_gallery" caption="`single` 版式的图像标题和元数据信息示例" %}

使用 `layout: single`，最好在 `_config.yml` 中设定 [Front Matter default]({{ "/docs/configuration/#front-matter-defaults" | relative_url }}) 参数来使用。

### 宽 Page

扩展主内容区一直到右侧，占据通常是页内目录的区域，只需添加下面的 YAML Front Matter：

```yaml
classes: wide
```

**备注：** 如果页面包含页内目录，它将不再出现在右侧，而是直接在主内容区的页面标题下方出现。
{: .notice--info}

### 页内目录

通过添加 `toc: true` 的 YAML Front Matter 系统就会自动生成页内目录列表。

![页内目录示例]({{ "/assets/images/mm-toc-helper-example.jpg" | relative_url }})

| 参数        | 是否必备  | 简介        | 默认值  |
| ---------   | -------- | ----------- | ------- |
| **toc**     | 可选 | 显示页内目录。（布尔值） | `false` |
| **toc_label** | 可选 | 页内目录标题。（字符串） | `toc_label` 在 UI 文本数据文件中设置。 |
| **toc_icon**  | 可选 | 页内目录图标，在标题前显示。（字符串） | [Font Awesome](https://fontawesome.com/v5/search?s=solid&m=free) <i class="fas fa-file-alt"></i> **file-alt** 图标。其它 FA 图标都可使用。 |
| **toc_sticky** | 可选 | 定位页内目录到屏幕顶端。 | `false` |
...
<figure>
  <img src="{{ '/assets/images/mm-layout-archive.png' | relative_url }}" alt="archive 版式示例">
  <figcaption>列表视图示例。</figcaption>
</figure>

下面是 archive 版式示例页，您可以直接在您的项目中使用，注意要根据您的站点要求修改 `permalink`、`title` 和文件名。每个版式都 100% 与 GitHub Pages 兼容。

* [所有 Post 通过分类进行分组 -- 列表视图][posts-categories]
* [所有 Post 根据 Tag 分组 -- 列表视图][posts-tags]
* [所有 Post 通过年分组 -- 列表视图][posts-year]
* [所有 Post 通过专题（Collection）分组 -- 列表视图][posts-collection]
* [Portfolio Collection -- 格栅分组][portfolio-collection]
...
Post 和 Page 摘要是由 Jekyll 抓取文字第一段落内容自动生成。要想设置指定文字可以通过使用下面的 YAML Front Matter 进行设定：
