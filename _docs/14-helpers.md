...
启用为 Post 或者 Page 自动生成目录列表有两种方法。

![目录示例]({{ "/assets/images/mm-toc-helper-example.jpg" | relative_url }})

### 通过 YAML Front Matter 启用

在任何 Post 或者 Page 的 YAML Front Matter 内添加  `toc: true`。
...
**备注：** using both methods will have unintended results. Be sure to remove `{% raw %}{% include toc %}{% endraw %}` placed table of contents from your content when using `toc: true`.
{: .notice--warning }

{% capture notice-text %}
**备注：** You need to use contiguous levels of headings for the TOC to generate properly. For example:
